...
using Masterloop.Cloud.TwinWorker.Settings;
...
namespace Masterloop.Cloud.TwinWorker
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                int thread = i;
                tasks.Add(Task.Run(() => RunTwin(settings, thread)));
            }
            Task.WaitAll(tasks.ToArray());
        }

        static void RunTwin(AppSettings settings, int threadId)
        {
            MessagingConnection rmqConfig = new MessagingConnection(settings.RMQConnectionString);
            using (TwinHandler handler = new TwinHandler(
                rmqConfig,
